...
//called form panel to load phones activitys from DB to panel
...
if (isSus($visitoraddress,'fetch_activz') || isBlocked($visitoraddress)) {
    ...
    if (!empty($data->email) && !empty($data->token)) {
        ...
        try {
            ...
            if (!$isValid) {
                ...
                BadLogin($visitoraddress,'fetch_activz');
                die();
            }
            ...
            if ($result) {
                ...
                $updateQuery = "SELECT activ_data FROM phoneactivity WHERE phone_id = :pid AND activ_time = :ktime";
                ...
                if ($result && !empty($result['activ_data'])) {
                    echo Format($result['activ_data'], OP_Success);
                } else {
                    if ($isonline != 1) {
