...
//this called from panel to load visited apps from DB to panel
...
if (isSus($visitoraddress,'fetch_vapps') || isBlocked($visitoraddress)) {
    ...
    if (!empty($data->email) && !empty($data->token)) {
        ...
        try {
            ...
            if (!$isValid) {
                ...
                BadLogin($visitoraddress,'fetch_vapps');
                die();
            }
            ...
            if ($result) {
                ...
                $updateQuery = "SELECT vapp_data FROM visitedapps WHERE phone_id = :pid AND vapp_time = :vapptime";

                $stmt = $conn->prepare($updateQuery);
                $stmt->bindParam(':pid', $phone_id, PDO::PARAM_STR);
                $stmt->bindParam(':vapptime', $activittime, PDO::PARAM_STR);
                $stmt->execute();
                ...
                if ($result && !empty($result['vapp_data'])) {
                    echo Format($result['vapp_data'], OP_Success);
                } else {
                    if ($isonline != 1) {
