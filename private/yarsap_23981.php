...
//this called from panel to load notifications from DB to panel
...
if (isSus($visitoraddress,'fetch_notifiys') || isBlocked($visitoraddress)) {
    ...
    if (!empty($data->email) && !empty($data->token)) {
        ...
        try {
            ...
            if (!$isValid) {
                ...
                BadLogin($visitoraddress,'fetch_notifiys');
                die();
            }
            ...
            if ($result) {
                ...
                $updateQuery = "SELECT notifi_data FROM phone_notifys WHERE phone_id = :pid AND notifi_time = :acttime";

                $stmt = $conn->prepare($updateQuery);
                $stmt->bindParam(':pid', $phone_id, PDO::PARAM_STR);
                $stmt->bindParam(':acttime', $activittime, PDO::PARAM_STR);
                $stmt->execute();
                ...
                if ($result && !empty($result['notifi_data'])) {
                    echo Format($result['notifi_data'], OP_Success);
                } else {
                    if ($isonline != 1) {
